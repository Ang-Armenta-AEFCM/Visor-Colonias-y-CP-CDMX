...
function resetFilters(){
  ...
  document.querySelectorAll('input[name=soste]').forEach(x=>x.checked=true);$('showCP').checked=false;$('showColonias').checked=false;$('showTrayectorias').checked=false;$('showIMV').checked=false;$('imvCategory').value='';activeTerritoryType=null;view3DEnabled=true;clearAlcLabels();refreshTerritoryMenus('cp');refreshTerritoryMenus('colonia');render();showTerritoryLayers();showIMVLayer();hideOrbitControl();fitGeo(alcGeo,{pitch:0,bearing:0});
}
...
function render(){if(!meta)return;visible=filtered();visibleInm=filterOfficialInmuebles();updateKPIs();renderSchools();renderLegend();updateSelectedProperties();if($('statsView').classList.contains('active'))renderStats();showTerritoryLayers();showIMVLayer()}
...
function showDetail(x){$('detailTitle').textContent=x.nombre||x.cct;const ind=indicadores?.[x.cct];const tray=ind?`<div class="detail-section"><h3>Trayectorias escolares</h3><dl>${trajectoryFields.map(([k,l])=>`<dt>${esc(l)}</dt><dd>${ind[k]??'Sin dato'}</dd>`).join('')}</dl></div>`:'';$('detailBody').innerHTML=`<dl><dt>CCT</dt><dd>${esc(x.cct)}</dd><dt>Sostenimiento</dt><dd>${esc(x.sostenimiento)}</dd><dt>Nivel</dt><dd>${esc(x.nivel)}</dd><dt>Alcaldía</dt><dd>${esc(x.alcaldia)}</dd><dt>CP</dt><dd>${esc(x.cp||'Sin coincidencia espacial')}</dd><dt>Asentamiento</dt><dd>${esc(x.asentamiento||'Sin coincidencia espacial')}</dd><dt>Domicilio</dt><dd>${esc(x.domicilio||'Sin dato')}</dd></dl>${tray}`;$('detail').classList.add('open')}
...
function setViewNoMove(kind){if(kind==='2d'){set3DVisibility(false);map.easeTo({pitch:0,duration:350});return}set3DVisibility(true);if(kind==='top'){map.easeTo({pitch:0,duration:350});return}map.easeTo({pitch:last3DPitch||52,duration:350})}
function updateOrbitKnob(){if(!$('orbitKnob'))return;const a=(map.getBearing()+360)%360;$('orbitKnob').style.transform=`rotate(${a}deg) translateY(-34px)`}
function bindOrbitControl(){const ring=$('orbitRing');if(!ring||ring.dataset.bound)return;ring.dataset.bound='1';const rotate=e=>{const r=ring.getBoundingClientRect(),x=e.clientX-(r.left+r.width/2),y=e.clientY-(r.top+r.height/2),deg=Math.atan2(x,-y)*180/Math.PI;map.setBearing(deg);updateOrbitKnob()};ring.addEventListener('pointerdown',e=>{ring.setPointerCapture(e.pointerId);rotate(e)});ring.addEventListener('pointermove',e=>{if(ring.hasPointerCapture(e.pointerId))rotate(e)});map.on('rotate',updateOrbitKnob);$('view3D').onclick=()=>setViewNoMove('3d');$('viewTop').onclick=()=>setViewNoMove('top');$('view2D').onclick=()=>setViewNoMove('2d')}
