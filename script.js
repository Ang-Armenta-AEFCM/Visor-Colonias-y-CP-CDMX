...
function installMapLayers(){
  if(!map.getSource('alcaldias')){
    ...
    map.addLayer({id:'inmuebles',type:'circle',source:'inmuebles',paint:{'circle-radius':6.2,'circle-color':['get','color'],'circle-stroke-width':1.4,'circle-stroke-color':'#fff','circle-opacity':.92}});
...
function bind(){
  ...
  $('inmBody').onclick=e=>{const b=e.target.closest('.open-cct');if(b){const c=schoolByCct.get(b.dataset.cct);if(c){$('inmModal').classList.remove('open');showDetail(c)}}};$('goMapInm').onclick=()=>{if(currentInm&&Number.isFinite(currentInm.lon)&&Number.isFinite(currentInm.lat)){switchView('mapView');map.easeTo({center:[currentInm.lon,currentInm.lat],zoom:17,pitch:45,duration:900});$('inmModal').classList.remove('open')}};
  $('basemap').onchange=()=>switchBasemap($('basemap').value);
  bindOrbitControl();
  $('inmuebleList').onclick=e=>{const r=e.target.closest('.inm-row');if(r){const x=schoolByCct.get(r.dataset.cct);if(x)showDetail(x)}};
}
...
function renderInmueblesMap(){
 ...
 const features=$('showInmuebles')?.checked?items.map(i=>{
  const peers=groups.get(i.lon.toFixed(7)+'|'+i.lat.toFixed(7)),angle=2*Math.PI*peers.indexOf(i)/peers.length;
  const radius=peers.length>1?0.000075:0;
  const students=i.ccts.map(cct=>schoolByCct.get(cct)).filter(x=>x&&shown.has(x.cct));
  const values=students.map(x=>indicadores?.[x.cct]?.[field]).filter(Number.isFinite);
  const v=values.length?Math.max(...values):null;
  const color=tray?trajectoryColor(v,field):colorFor(students[0]||{sostenimiento:'Público',nivel:''},$('colorMode').value);
  return {type:'Feature',geometry:{type:'Point',coordinates:[i.lon+Math.cos(angle)*radius,i.lat+Math.sin(angle)*radius]},properties:{key:i.key,id:i.id,ccts:i.ccts?.length||0,color}};
 }):[];
 map.getSource('inmuebles').setData({type:'FeatureCollection',features});
}
...
async function showDetail(x){
  ...
  if(ind&&trajectoryFields.some(([k])=>clean(x.nivel).includes(k.split('_').at(-1).toUpperCase()))){
    const fields=trajectoryFields.filter(([k])=>clean(x.nivel).includes(k.split('_').at(-1).toUpperCase()));
    const rows=fields.map(([k,l])=>({k,l,v:Number.isFinite(ind[k])?Number(ind[k]):null})),available=rows.filter(r=>r.v!==null),abandono=rows.filter(r=>r.k.startsWith('abandono_')&&r.v!==null).reduce((a,r)=>a+r.v,0),noProm=rows.filter(r=>r.k.startsWith('no_promovidos_')&&r.v!==null).reduce((a,r)=>a+r.v,0),total=available.reduce((a,r)=>a+r.v,0),maxRow=available.length?available.reduce((a,b)=>a.v>=b.v?a:b):null;
    tray=`<div class="detail-section"><h3>Resumen de trayectorias escolares</h3><div class="trajectory-summary-grid"><div><strong>${available.length}/${rows.length}</strong><span>Variables con dato</span></div><div><strong>${fmt(total)}</strong><span>Recuento total reportado</span></div><div><strong>${fmt(abandono)}</strong><span>Abandono acumulado</span></div><div><strong>${fmt(noProm)}</strong><span>No promovidos acumulados</span></div></div>${maxRow?`<p class="summary-total"><b>Mayor recuento:</b> ${esc(maxRow.l)} · ${fmt(maxRow.v)}</p>`:''}<dl>${rows.map(r=>`<dt>${esc(r.l)}</dt><dd>${r.v===null?'Sin dato':fmt(r.v)}</dd>`).join('')}</dl></div>`;
  }else tray='<div class="detail-section"><h3>Resumen de trayectorias escolares</h3><p class="hint">Este CCT no tiene indicadores de trayectorias para su nivel educativo.</p></div>';
...
function showInmueble(key){const i=inmuebles.find(x=>x.key===key)||visibleInm.find(x=>x.key===key);if(!i)return;currentInm=i;$('inmTitle').textContent=`${i.id} · ${i.ccts.length} CCT`;$('inmBody').innerHTML=`<div class="inm-meta"><b>Alcaldía</b><span>${esc(i.alcaldia||'—')}</span><b>CP</b><span>${esc(i.cp||'—')}</span><b>Colonia</b><span>${esc(i.asentamiento||'—')}</span><b>Domicilio</b><span>${esc(i.domicilio||'—')}</span><b>Coordenadas</b><span>${Number.isFinite(i.lat)&&Number.isFinite(i.lon)?`${i.lat.toFixed(6)}, ${i.lon.toFixed(6)}`:'Sin coordenadas'}</span></div>${i.ccts.length?i.ccts.map(cct=>{const c=schoolByCct.get(cct),ind=indicadores?.[cct];return c?`<div class="cct-detail"><button type="button" class="open-cct" data-cct="${esc(c.cct)}">${esc(c.cct)} · ${esc(c.nombre)}</button><p><b>Nivel:</b> ${esc(c.nivel)} · <b>Sostenimiento:</b> ${esc(c.sostenimiento)}</p>${ind?`<p><b>Trayectorias:</b> ${trajectoryFields.filter(([k])=>ind[k]!=null&&clean(c.nivel).includes(k.split('_').at(-1).toUpperCase())).map(([k,l])=>`${esc(l)}: ${ind[k]}`).join(' · ')||'Sin datos'}</p>`:''}</div>`:`<div class="cct-detail"><h4>${esc(cct)}</h4><p>CCT del inmueble sin registro coincidente en la base completa.</p></div>`}).join(''):'<div class="cct-detail"><p>Inmueble oficial sin CCT asociado en esta versión de la base.</p></div>'}`;$('goMapInm').disabled=!(Number.isFinite(i.lat)&&Number.isFinite(i.lon));$('inmModal').classList.add('open')}
